...
var logFile = path.join(logsDir, "app.log");
...
function parseTrackMetadata(videoTitle, channelName) {
  ...
  let clean = videoTitle.replace(/\s+-\s*/g, " - ").replace(/\s*-\s+/g, " - ");
  clean = clean.replace(/\[\s*(official\s+video|official\s+audio|official\s+music\s+video|official\s+lyric\s+video|lyric\s+video|lyrics|official|audio|video|hd|hq|1080p|4k|visualizer|clip\s+officiel)\s*\]/gi, "").replace(/\(\s*(official\s+video|official\s+audio|official\s+music\s+video|official\s+lyric\s+video|lyric\s+video|lyrics|official|audio|video|hd|hq|1080p|4k|visualizer|clip\s+officiel)\s*\)/gi, "").trim();
  const separators = [" - ", " \u2013 ", " \u2014 ", " | ", " : ", " / "];
  ...
  const isGenericChannel = lowerChannel.includes("topic") || lowerChannel.includes("lyrics") || lowerChannel.includes("music") || lowerChannel.includes("vevo") || lowerChannel.includes("records") || lowerChannel.includes("studio") || lowerChannel.includes("uploads") || lowerChannel.includes("soundtrack") || lowerChannel.includes("channel") || lowerChannel.includes("mp3") || lowerChannel.includes("karaoke") || lowerChannel.includes("sound");
  if (!artist || !songTitle || artist.length > 50) {
    ...
    const isArtistGeneric = parsedArtistLower.includes("topic") || parsedArtistLower.includes("lyrics") || parsedArtistLower.includes("music") || parsedArtistLower.includes("vevo") || parsedArtistLower.includes("records") || parsedArtistLower.includes("studio") || parsedArtistLower.includes("uploads") || parsedArtistLower.includes("soundtrack") || parsedArtistLower.includes("channel") || parsedArtistLower.includes("sound") || parsedArtistLower.length > 35;
...
function getNormalizedString(str) {
  if (!str) return "";
  return str.toLowerCase().replace(/[^a-z0-9\u0600-\u06FF]/g, "").replace(/official/g, "").replace(/video/g, "").replace(/audio/g, "").replace(/lyrics/g, "").replace(/lyric/g, "").replace(/original/g, "").replace(/full/g, "").replace(/song/g, "").trim();
}
...
app.get("/api/suggest", async (req, res) => {
  try {
    ...
    const videos = r.videos.filter((v) => !excludeSet.has(v.videoId) && v.seconds > 60 && v.seconds < 600).slice(0, 5).map((v) => {
      const parsed = parseTrackMetadata(v.title, v.author?.name);
      return {
        id: v.videoId,
        title: parsed.title,
...
`);
    ...
    try {
      const [remixRes, similarRes] = await Promise.all([
        yts(remixQuery).catch((err) => {
          console.warn("[SMART-SUGGEST] remix query failed:", err.message);
          return { videos: [] };
        }),
        yts(similarQuery).catch((err) => {
          console.warn("[SMART-SUGGEST] similar query failed:", err.message);
          return { videos: [] };
        })
      ]);
  ...
  for (const model of modelsToTry) {
    ...
    try {
      const prompt = [
        ...
        ``,
        `Requirements:`,
        `- The first 7 songs (index 0-6) MUST be standard matching tracks: similar genre, vibe, energy level, and BPM (within \xB110%).`,
  ...
  for (const song of aiSongs.slice(0, 10)) {
    ...
    try {
      ...
      const video = r.videos?.find(
        (v) => !excludeSet.has(v.videoId) && v.seconds > 60 && v.seconds < 600
      );
      if (video) {
